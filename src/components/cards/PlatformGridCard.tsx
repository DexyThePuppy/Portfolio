...
import type { PlatformItem } from '../../data/profileData';
...
const PlatformGridCard: React.FC<PlatformGridCardProps> = React.memo(({ platform, isHovered, setHoveredId }) => {
  ...
  const {
    ...
    handleFocus,
    handleBlur,
  } = useGridCardHover(`platform-${platform.id}`, setHoveredId);
        ...
        >
          ...
          >
            ...
            <div className="min-w-0">
              <h4 className="text-sm font-bold text-on-background leading-tight truncate">{platform.name}</h4>
              <p className="text-xs text-on-surface-variant leading-tight truncate">{platform.description}</p>
            </div>
          </div>
        </a>
      </GridCard>
...
export default PlatformGridCard;
